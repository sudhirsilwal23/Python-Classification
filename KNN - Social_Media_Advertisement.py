#################################################################################
#####################  K-Nearest Neighbours (K-NN) - Python  #####################
#################################################################################
...
plt.contourf(X1, X2, Var_KNN.predict(np.array([X1.ravel(), X2.ravel()]).T).reshape(X1.shape),
...
plt.title('Social Media Advertisement K-NN (Training set)')
plt.xlabel('Age')
...
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(Var_I_set[Var_D_set == j, 0], Var_I_set[Var_D_set == j, 1],
                c = ListedColormap(('red', 'green'))(i), label = j)
plt.title('Social Media Advertisement K-NN (Test set)')
plt.xlabel('Age')
plt.ylabel('Estimated Salary')
plt.legend()
plt.show()
