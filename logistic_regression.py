...
Var_LR.fit(Var_I_train, Var_D_train)

        #3(b) Predicting the Test set results
Var_D_pred = Var_LR.predict(Var_I_test)
...
plt.contourf(X1, X2, Var_LR.predict(np.array([X1.ravel(), X2.ravel()]).T).reshape(X1.shape),
             alpha = 0.75, cmap = ListedColormap(('red', 'green')))
plt.xlim(X1.min(), X1.max())
plt.ylim(X2.min(), X2.max())
for i, j in enumerate(np.unique(Var_D_set)):
    plt.scatter(Var_I_set[Var_D_set == j, 0], Var_I_set[Var_D_set == j, 1],
                c = ListedColormap(('red', 'green'))(i), label = j)
plt.title('Social Media Advertisement Logistic Regression (Training set)')
plt.xlabel('Age')
...
for i, j in enumerate(np.unique(Var_D_set)):
    plt.scatter(Var_I_set[Var_D_set == j, 0], Var_I_set[Var_D_set == j, 1],
                c = ListedColormap(('red', 'green'))(i), label = j)
plt.title('Social Media Advertisement Logistic Regression (Test set)')
plt.xlabel('Age')
plt.ylabel('Estimated Salary')
plt.legend()
plt.show()